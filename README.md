...
- feat(android): 优化键间距与候选拼音标签，加入英文单次 Shift、红色立体操作/符号键，以及位于最左侧的标准系统输入法切换键。
- feat(windows): 完整接入 TSF UI-less 搜索候选协议，同时恢复浏览器、微信等普通应用的原生候选窗与组合输入。
- fix(ci): 修复质量门禁与 Windows 链接检查。

## 目录

- [赣语通用输入法 GonnyuGeneralIME](#赣语通用输入法-gonnyugeneralime)
  - [测试版本 0.2.4-pre.15](#测试版本-024-pre15)
  - [目录](#目录)
  - [简介](#简介)
    - [特色](#特色)
  - [功能演示](#功能演示)
...
**声明**：我们重点对词典文件维护拼音准确性，但暂时不能对ipa部分数据一致性/准确性做出保证，因为后者只是词典构建时辅助导入，与用户可见输入无关。
