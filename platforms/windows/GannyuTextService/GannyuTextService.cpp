...
static const GUID kSupportedCategories[] = {
    ...
    // Allow TSF to activate the in-process keyboard TIP on host threads that
    // do not initialize COM themselves.
    GUID_TFCAT_TIPCAP_COMLESS,
    // Keep per-thread Chinese/English state in TSF input-mode compartments.
...
public:
    ...
    STDMETHODIMP ActivateEx(ITfThreadMgr *mgr, TfClientId clientId, DWORD flags) override {
        ...
        if (SUCCEEDED(threadMgr_->QueryInterface(IID_ITfThreadMgr2,
            ...
            threadMgr2) {
            threadMgr2->GetActiveFlags(&activeFlags);
            threadMgr2->Release();
        }
        const DWORD effectiveFlags = flags | activeFlags;
        uiLessMode_ = (effectiveFlags & TF_TMF_UIELEMENTENABLEDONLY) != 0;
        immersiveMode_ = (effectiveFlags & TF_TMF_IMMERSIVEMODE) != 0;
...
private:
    ...
    bool EnsureCandidateWindow() {
        // Immersive hosts require the candidate popup to belong to the active
        // context view so it remains in the host's visible presentation layer.
        HWND ownerWindow = nullptr;
        if (immersiveMode_ && TryGetContextViewWindow(&ownerWindow)) {
            HWND root = GetAncestor(ownerWindow, GA_ROOT);
            if (root) ownerWindow = root;
            DWORD ownerProcessId = 0;
            GetWindowThreadProcessId(ownerWindow, &ownerProcessId);
            if (ownerProcessId != GetCurrentProcessId()) ownerWindow = nullptr;
        }
        if (candidateWindow_) {
            if (immersiveMode_ && ownerWindow && candidateOwnerWindow_ != ownerWindow) {
    ...
    HWND candidateOwnerWindow_ = nullptr;
