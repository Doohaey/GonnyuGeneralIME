...
rem MSI compares only major.minor.build, so encode the patch and pre-release
rem sequence into build.  The Burn bundle may retain its four-part display version.
rem  Pre     X.Y.Z-pre.N -> MSI X.Y.(Z*1001+N), Bundle X.Y.Z.N
rem  Stable  X.Y.Z       -> MSI X.Y.(Z*1001+1000), Bundle X.Y.Z.65535
set "PS_OUT=%TEMP%\gannyu_ver_%RANDOM%.out"
powershell -NoProfile -Command "$v='%GONNYU_VERSION%'; $pre=[regex]::Match($v,'^(?<major>\d+)\.(?<minor>\d+)\.(?<patch>\d+)-pre\.(?<sequence>\d+)$'); if($pre.Success){$major=[int]$pre.Groups['major'].Value;$minor=[int]$pre.Groups['minor'].Value;$patch=[int]$pre.Groups['patch'].Value;$n=[int]$pre.Groups['sequence'].Value;$build=$patch*1001+$n;if($n -ge 1000 -or $build -gt 65535){exit 1};$msiVersion='{0}.{1}.{2}' -f $major,$minor,$build;$bundleVersion='{0}.{1}.{2}.{3}' -f $major,$minor,$patch,$n;Write-Output ($msiVersion+'|'+$bundleVersion)} else {$stable=[regex]::Match($v,'^(?<major>\d+)\.(?<minor>\d+)\.(?<patch>\d+)$');if(!$stable.Success){exit 1};$major=[int]$stable.Groups['major'].Value;$minor=[int]$stable.Groups['minor'].Value;$patch=[int]$stable.Groups['patch'].Value;$build=$patch*1001+1000;if($build -gt 65535){exit 1};$msiVersion='{0}.{1}.{2}' -f $major,$minor,$build;$bundleVersion='{0}.{1}.{2}.65535' -f $major,$minor,$patch;Write-Output ($msiVersion+'|'+$bundleVersion)}" > "%PS_OUT%"
if errorlevel 1 (
